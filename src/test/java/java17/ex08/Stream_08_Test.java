...
public class Stream_08_Test {
    ...
    public void test_group() throws IOException {
        ...
        try (Stream<String> lines = Files.lines(Paths.get(NAISSANCES_DEPUIS_1900_CSV))) {
            ...
            Map<String, Integer> result = lines.skip(1).collect(Collectors.groupingBy(line -> line.substring(line.indexOf(";") + 1, line.indexOf(";") + 5), Collectors.summingInt(line -> Integer.parseInt(line.substring(line.lastIndexOf(';') + 1)))));
    ...
    public void test_max() throws IOException {
        ...
        try (Stream<String> lines = Files.lines(Paths.get(NAISSANCES_DEPUIS_1900_CSV))) {
            ...
            Optional<Naissance> result = lines.skip(1)
                                              .map(line -> new Naissance(line.substring(line.indexOf(";") + 1, line.indexOf(";") + 5), line.substring(line.indexOf(";") + 6, line.lastIndexOf(";")), Integer.parseInt(line.substring(line.lastIndexOf(';') + 1))))
    ...
    public void test_collectingAndThen() throws IOException {
        ...
        try (Stream<String> lines = Files.lines(Paths.get(NAISSANCES_DEPUIS_1900_CSV))) {
            ...
            Map<String, Naissance> result = lines.skip(1)
                    .map(line -> new Naissance(line.substring(line.indexOf(";") + 1, line.indexOf(";") + 5), line.substring(line.indexOf(";") + 6, line.lastIndexOf(";")), Integer.parseInt(line.substring(line.lastIndexOf(';') + 1))))
                    .collect(Collectors.groupingBy(Naissance::getAnnee, Collectors.collectingAndThen(Collectors.maxBy(Comparator.comparingInt(Naissance::getNombre)), Optional::get))
                    );
