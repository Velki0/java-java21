...
public class Stream_03_Test {
	...
	public void test_flatMap() throws Exception {
		...
		List<Pizza> result = orders.stream().map(Order::getPizzas).flatMap(List::stream).toList();
	...
	public void test_flatMap_distinct() throws Exception {
		...
		List<Pizza> result = orders.stream().map(Order::getPizzas).flatMap(List::stream).distinct().toList();
	...
	public void test_partitionning() throws Exception {
		...
		Map<Boolean, List<Pizza>> result = pizzas.stream().collect(Collectors.partitioningBy(pizza -> pizza.getName().charAt(0) == 'L'));
